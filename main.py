...
def game_loop():
    ...
    while running:
        ...
        if pygame.time.get_ticks() - lastenhancetime < 2000:
            screen.blit(choosefont(30).render(f'{upgrademessage}',True,(255,0,0)),(20,730))
        ...
        if random.random() < enemyspawn:  # 랜덤 모듈로 생성한 난수가 enemyspawn보다 작을때 적 생성
            ...
            if Enemy.num %50 == 0:# 적 50마리 생성될때 마다 보스 몹 소환
                enemies.append(Enemy(0,100,min(1300000,1500+50*(2**Enemy.bosscount)),2,min(6000,Enemy.bosscount*500),'boss'))#적이 생성된 횟수가 많을 수록 체력이 많아짐 --> 점점 난이도 상승
                Enemy.bosscount += 1
            else:#평상시엔 평범한 몹
                enemies.append(Enemy(0, 100,int(600+10*((Enemy.num*10)**0.6)) ,4, min(2000,100+30*(Enemy.num//50)),'common'))#적이 생성된 횟수가 많을 수록 체력이 많아짐 --> 점점 난이도 상승
        ...
        enemyhp = choosefont(30).render(f'common hp : {int(600+10*((Enemy.num*10)**0.6))}',True,(0,0,0))
